...
class SegDetTrainer(BaseTrainer):
    ...
    def loss(self, predictions: Dict, batch: Dict) -> Dict[str, torch.Tensor]:
        ...
        gt_obj, gt_box, box_mask = self._prepare_targets(batch, (grid_h, grid_w))
        ...
        pred_wh = torch.exp(torch.clamp(det_logits[:, 2:4, :, :], min=-5, max=5))
        ...
        total = w_seg * loss_seg + w_obj * loss_obj + w_box * loss_box
